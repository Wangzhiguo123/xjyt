    ...
    >
      <div class="content">
        ...
        <el-form label-width="80px" :model="formInline" ref="formInline" :rules="rules">
                 <el-form-item label="车辆颜色" prop="name">
...
<script>
import { carColorAdd,carColorEdit } from "@/api/modules/carManagement";
export default {
  data() {
    return {
      formInline:{
  ...
  methods: {
    ...
    submitForm(formInline) {
            ...
            if (valid) {
                ...
                let res = ''
                if (this.title === '新增') {
                     res = await carColorAdd(params);
                } else {
                     res = await carColorEdit(params);
                }
                if (res.status === 204) {
