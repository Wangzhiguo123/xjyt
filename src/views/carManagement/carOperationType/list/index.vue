...
import { carOperationTypesPage,carOperationTypesDel } from "@/api/modules/carManagement";
import addOrEdit from "../addOrEdit";
import { tbColumnCon } from "./config";
export default {
  name: "carOperationTypes",
  components: {
    tableCom,
    addOrEdit,
    delModal
  ...
  methods: {
    ...
    async delConfirm (data) {
      ...
      let res = await carOperationTypesDel(data);
      console.log('res',res)
      if (res.status === 204) {
        this.queryList();
      } else {
    ...
    async queryList() {
      ...
      let res = await carOperationTypesPage(params);
