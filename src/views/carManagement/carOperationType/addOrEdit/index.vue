    ...
    >
      <div class="content">
        ...
        <el-form label-width="120px" :model="formInline" ref="formInline" :rules="rules">
                 <el-form-item label="车辆运营类型" prop="name">
                                <el-input v-model="formInline.name"/></el-form-item>
                 <el-form-item label="运营类型编码"  prop="code">
...
<script>
import { carOperationTypesAdd,carOperationTypesEdit } from "@/api/modules/carManagement";
export default {
  data() {
    return {
      formInline:{
  ...
  methods: {
    ...
    submitForm(formInline) {
            ...
            if (valid) {
                ...
                let res = ''
                if (this.title === '新增') {
                     res = await carOperationTypesAdd(params);
                } else {
                     res = await carOperationTypesEdit(params);
                }
                if (res.status === 204) {
