  ...
  <div class="addOrEdit">
    ...
    >
      <div class="content">
        ...
        <el-form label-width="120px" :model="formInline" ref="formInline" :rules="rules">
                 <el-form-item label="车辆运维类型" prop="name">
                                <el-input v-model="formInline.name"/></el-form-item>
                 <el-form-item label="运维类型编码"  prop="code">
...
<script>
import { carOperationStatussAdd,carOperationStatussEdit } from "@/api/modules/carManagement";
export default {
  data() {
    return {
      formInline:{
  ...
  methods: {
    ...
    submitForm(formInline) {
            ...
            if (valid) {
                ...
                let res = ''
                if (this.title === '新增') {
                     res = await carOperationStatussAdd(params);
                } else {
                     res = await carOperationStatussEdit(params);
                }
                if (res.status === 204) {
