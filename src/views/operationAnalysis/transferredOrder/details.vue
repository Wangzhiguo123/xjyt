...
import { processTransactionPage,Create,Deletes,Updates } from "@/api/modules/dictionaryDetails";
export default {
  name: "Home",
  components: {
    Pages,
  ...
  // },
    data() {
      return {
        ...
        the_level:'',
        data:{
          orderType:'dictionaryDetail',//字典类别
          total:'',
          every_page:'',
          search:true,
          edits:true,
  ...
  methods: {
    ...
    async d_action(type,newdata,index){
      if(type=='dictionary_add'){
        ...
        let thisData = newdata[index].dictionaryId
