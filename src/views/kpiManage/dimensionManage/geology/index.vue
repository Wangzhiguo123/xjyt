 ...
 * @LastEditTime: 2021-09-15 16:51:25
-->
<template>
  <div class="container">
    <el-button type="primary" v-if="hasShowInit" @click="addInit">初始化地质关系维度</el-button>
    <el-table
      :data="tableData"
      style="width: 300px;"
      default-expand-all
    ...
    >
      <el-table-column prop="label" label="地质关系维度"> </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    addInit(){
      ...
      this.addPoint({ key:"sibling", label: "初始化地质关系维度"});
    },
    /**
     * @description: 获取场景列表
     * @param {*}
    ...
    deletePoint() {
      this.$confirm("是否确认删除节点?", "提示", {
        ...
        let { data } = await deleteGeology(this.rowTarget.id);
        if (data.code === undefined) {
          this.getGeologyList();
          this.isContextShow = false;
          this.$message({
