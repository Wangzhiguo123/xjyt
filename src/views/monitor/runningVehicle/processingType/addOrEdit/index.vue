    ...
    >
      <div class="content">
        ...
        <el-form label-width="140px" :model="formInline" ref="formInline" :rules="rules">
                 <el-form-item label="车辆告警处理方式" prop="name">
                                <el-input v-model="formInline.name"/></el-form-item>
                 <el-form-item label="告警处理方式编码"  prop="code">
...
<script>
import { handlesTypesAdd,handlesTypesEdit } from "@/api/modules/productionMonitoring";
export default {
  data() {
    return {
      formInline:{
  ...
  methods: {
    ...
    submitForm(formInline) {
            ...
            if (valid) {
                ...
                let res = ''
                if (this.title === '新增') {
                     res = await handlesTypesAdd(params);
                } else {
                     res = await handlesTypesEdit(params);
                }
                if (res.status === 204) {
