...
<script>
import { processingStatusAdd,processingStatusEdit } from "@/api/modules/productionMonitoring";
export default {
  data() {
    return {
      formInline:{
  ...
  methods: {
    ...
    submitForm(formInline) {
            ...
            if (valid) {
                ...
                let res = ''
                if (this.title === '新增') {
                     res = await processingStatusAdd(params);
                } else {
                     res = await processingStatusEdit(params);
                }
                if (res.status === 204) {
